...
if (registerForm) {
    ...
    latitude: parseFloat(document.getElementById('lat').value),   // ADD THIS
    longitude: parseFloat(document.getElementById('lng').value),  // ADD THIS
    rating: 5.0
};
...
async function fetchGuides(city = "") {
    try {
        ...
        let url = '/api/guides';
        if (city) {
            url = `/api/guides/search?city=${city}`;
        }

        const response = await fetch(url);
        const guides = await response.json();
        
        // 2. Clear existing pins from map (optional but better)
        if (map) {
            map.eachLayer((layer) => {
        ...
        // 3. Initialize Map if it doesn't exist
        if (!map) {
        ...
        if (guides.length > 0 && guides[0].latitude) {
